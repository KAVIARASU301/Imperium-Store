  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (mode === "reset") {
        ...
        if (error) throw error;
        setStatus({ tone: "success", text: "Password updated. Redirecting..." });
        router.push(next);
        return;
      }
      ...
      if (mode === "signup") {
        ...
        if (error) throw error;
        if (data.session) {
          setStatus({ tone: "success", text: "Account created. Redirecting..." });
          router.push(next);
          return;
        }
      ...
      <main className="mx-auto grid max-w-6xl gap-10 px-6 py-16 lg:grid-cols-[0.92fr_1.08fr] lg:py-24">
        ...
        <section className="rounded-3xl border border-slate-800 bg-[#0B1020]/90 p-6 shadow-2xl shadow-cyan-950/20 sm:p-8">
          <div className="flex rounded-full border border-slate-800 bg-black/20 p-1 text-sm font-semibold">
            <button type="button" onClick={() => switchMode("signin")} className={`flex-1 rounded-full px-4 py-2 ${mode === "signin" ? "bg-cyan-300 text-black" : "text-slate-400 hover:text-white"}`}>
              Login
            </button>
            <button type="button" onClick={() => switchMode("signup")} className={`flex-1 rounded-full px-4 py-2 ${mode === "signup" ? "bg-cyan-300 text-black" : "text-slate-400 hover:text-white"}`}>
              Sign up
            </button>
          </div>
              ...
              <div className="mt-6 grid gap-3 sm:grid-cols-2">
                {oauthProviders.map(({ provider, label, icon }) => (
                    <button
                        ...
                        className="flex items-center justify-center gap-3 rounded-2xl border border-slate-700 bg-white px-4 py-3 font-semibold text-slate-950 hover:bg-cyan-50 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                ...
                <label className="block text-sm font-medium text-slate-300">
                  ...
                  <div className="mt-2 flex rounded-2xl border border-slate-800 bg-black/20 focus-within:border-cyan-300">
                    <input
                        ...
                        required
                    />
                    <button type="button" onClick={() => setShowPassword((current) => !current)} className="px-4 text-sm font-semibold text-cyan-300">
          ...
          <div className="mt-6 flex flex-wrap items-center justify-between gap-3 text-sm text-slate-400">
            {mode !== "forgot" && mode !== "reset" ? (
                ...
                <button type="button" onClick={() => switchMode("signin")} className="font-semibold text-cyan-300 hover:text-cyan-200">
                  Back to login
                </button>
            )}
            <span>
            Protected by <Link href="/privacy-policy" className="text-cyan-300 hover:text-cyan-200">privacy-first policies</Link>.
          </span>
          </div>
        </section>
      </main>
