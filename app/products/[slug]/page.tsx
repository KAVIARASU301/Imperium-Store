          ...
          {product.badges?.length ? (
            <div className="mt-6 flex flex-wrap gap-2">
              {product.badges.map((badge) => (
                <span key={badge} className="border border-cyan-300/30 bg-cyan-300/10 px-3 py-1.5 font-mono text-xs uppercase tracking-wider text-cyan-200">
                  {badge}
                </span>
              ))}
            </div>
...
function Highlights({ items }: { items: ProductHighlight[] }) {
  return (
    <section className="mt-10 grid gap-3 md:grid-cols-3">
      {items.map((item) => (
        <div key={item.title} className="border border-slate-800 bg-[#0B1020] p-4">
  ...
  return (
    <section className="mt-12">
      ...
      <div className="mt-5 grid gap-5">
        {items.map((item) => (
          <figure key={item.src} className="overflow-hidden border border-slate-800 bg-[#0B1020]">
